...
public class LanguageLua extends Language {
    ...
    private final static String package_table = "maxn|foreachi|concat|clone|size|sort|move|find|pack|remove|gfind|unpack|insert|const|foreach|clear";//"clear|clone|concat|const|find|foreach|foreachi|gfind|insert|maxn|move|pack|remove|size|sort|unpack|keys|vals|add|fill";
    ...
    function a(keys)
  function b(key)
    local s = "private final static String package_"..key.." = \""
    for k, v pairs(_G[key])
      s = s..k.."|"
    end
    s = s:sub(0, #s - 1) .. "\";"
    return s
...
a({"coroutine","debug","io","luajava","math","os","package","string","table","utf8"})
   */
    
    private static final String package_coroutine =
            "status|running|resume|yield|create|wrap|isyieldable";
    private static final String package_debug =
            "getlocal|setuservalue|sethook|debug|gethook|upvalueid|getfenv|getmetatable|setfenv|traceback|setlocal|setmetatable|upvaluejoin|setupvalue|getinfo|getupvalue|getuservalue|getregistry";
    private static final String package_io =
            "tmpfile|write|close|stderr|lines|flush|open|read|stdin|popen|input|stdout|output|type";
    private static final String package_luajava =
            "_NAME|_VERSION|newInstance|loadLib|astable|loaded|_COPYRIGHT|new|createProxy|ids|getContext|luapath|luadir|luaextdir|tostring|createArray|bindClass|_MOD|_DESCRIPTION|instanceof|newArray|imported|clear|coding|override";
    private static final String package_math =
            "sqrt|cos|ceil|acos|deg|asin|sinh|log|tointeger|atan|modf|max|huge|fmod|tan|frexp|randomseed|sin|mininteger|maxinteger|pi|ldexp|min|cosh|log10|pow|random|tanh|atan2|rad|abs|exp|floor|ult|type";
    private static final String package_os =
            "tmpname|remove|rename|time|getenv|date|execute|setlocale|clock|difftime|exit";
    private static final String package_package =
            "searchpath|path|loadlib|loaded|seeall|config|loaders|cpath|searchers|preload";
    private static final String package_string =
            "unpack|packsize|lower|sub|byte|gsub|gfind|char|len|format|match|dump|rep|pack|find|upper|reverse|gmatch";
    private static final String package_table =
            "foreachi|unpack|sort|move|pack|remove|foreach|insert|const|maxn|concat|find|clear|gfind|clone|size";
    private static final String package_utf8 =
            "offset|lower|codepoint|byte|charpattern|gsub|fold|find|char|codes|len|reverse|match|gmatch|widthindex|title|ncasecmp|remove|escape|next|charpos|width|insert|upper|sub";
