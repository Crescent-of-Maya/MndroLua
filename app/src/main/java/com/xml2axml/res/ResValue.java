...
public class ResValue {
    ...
    public static boolean stringToValue(Context context, AttributeEntry entry, String[] defPackage) throws Exception{
        ...
        if (s[0] == '@') {
            ...
            if (len == 5 && s[1] == 'n' && s[2] == 'u' && s[3] == 'l' && s[4] == 'l') {
                ...
                try {
                    // 7fxxxxxx
                    outValue.data = Integer.valueOf(entry.string.substring(1, entry.string.length()), 16);
                    // 类型: 资源ID
                    outValue.dataType = ResValue.TYPE_REFERENCE;
                    return true;
                } catch(Exception e) {
                ...
                // Log.e("lua", entry.string);
                /*try {
                    outValue.data = Integer.valueOf(entry.string.substring(1, entry.string.length()));
                    return true;
                } catch(Exception e) {
    ...
    private static int getResourcePackageIndex(Context context, int resID) throws Exception {
        if (resPackages == null) {
            ...
            Object o = HiddenApiBypass.invoke(AssetManager.class, assetManager, "getAssignedPackageIdentifiers", new Object[]{});
            if (o != null) {
    ...
    private static int lockBag(Context context, int resID, ArrayList<ResMap> bag) throws Exception {
        AssetManager assetManager = context.getAssets();
        int[] o = (int[]) HiddenApiBypass.invoke(AssetManager.class, assetManager, "getStyleAttributes", resID);
        if (o == null) {
            return -75;//ERROR
        }
        Resources resources = context.getResources();
        TypedArray array = resources.obtainTypedArray(resID);
        int[] name_ident_array = o;
    ...
    private static boolean stringToInt(char[] s, int len, ResValue outValue) {
        ...
        if (len > 1 && s[i] == '0' && s[i + 1] == 'x') {
            ...
            if (i == len) {
                ...
                /*if (outValue != null) {
                    try {
                        outValue.data = Integer.valueOf(new String(s));
                        outValue.dataType = ResValue.TYPE_INT_DEC;
                    } catch(Exception e) {
                        try {
                            outValue.data = Integer.valueOf(new String(s), 16);
                            outValue.dataType = ResValue.TYPE_INT_HEX;
                        } catch(Exception e2) {
                            return false;
                        }
        ...
        // 2025.1.8
        // 这到底有什么用啊
        // 修复: 如 11000 34 等int数字无法被识别的问题
        // 识别失败就会转换成float(不该被转换的类型)
        // 例如 versionCode = 114514.0
        // 坑死我了
        /*
        while (i < len && s[i] <= 32) {
            i++;
        ...
        if (i != len) {
            // Log.e("lua", new String(s) + " i=" + i + " len=" + len);
            return false;
        }*/
