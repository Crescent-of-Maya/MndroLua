...
public class LuaApplication extends Application implements LuaContext {
    ...
    public void onCreate() {
        ...
        //初始化AndroLua工作目录
        // 仅编辑器需要这些
        if ("io.github.moonleeeaf.androlua".equals(getPackageName())) {
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String sdDir = Environment.getExternalStorageDirectory().getAbsolutePath();
                luaExtDir = sdDir + "/Documents/MndroLua";
            } else {
                ...
                for (File f : fs) {
                    ...
                    if (ls == null)
                        continue;
                    if (ls.length > 5)
                        luaExtDir = f.getAbsolutePath() + "/Documents/MndroLua";
                }
                if (luaExtDir == null)
                    luaExtDir = getDir("MndroLua", Context.MODE_PRIVATE).getAbsolutePath();
            }
		...
		//Toast.makeText(this,""+libDir+"==="+luaMdDir,1).show();
