...
local function binapk(luapath, apkpath)
    ...
    if f.isDirectory() then
        ...
        if wel.exists() then
            local entry = ZipEntry("res/drawable/ic_launcher.png")
            out.putNextEntry(entry)
            replace["res/drawable/ic_launcher.png"] = true
            copy(FileInputStream(wel), out)
        end
        local wel = File(luapath .. "icon_round.png")
        if wel.exists() then
            local entry = ZipEntry("res/drawable/ic_launcher_round.png")
            out.putNextEntry(entry)
            replace["res/drawable/ic_launcher_round.png"] = true
            copy(FileInputStream(wel), out)
        elseif File(luapath .. "icon.png").exists() then
            local wel = File(luapath .. "icon.png")
            -- 没有圆角的图标 那就回滚 :)
            local entry = ZipEntry("res/drawable/ic_launcher_round.png")
            out.putNextEntry(entry)
    ...
    while entry do
        ...
        elseif name == "" then
            ...
            --luajava.bindClass("android.util.Log").e("lua", name)
            local entry = ZipEntry(name)
            out.putNextEntry(entry)
            if entry.getName() == "AndroidManifest.xml" then
                ...
                for n = 0, list.size() - 1 do
                    ...
                    if req[v] then
                        list.set(n, req[v])
                    elseif tostring(v):find(activity.getPackageName())
                        -- 万用方案
                        -- gsub返回的是unpack的值 必须截断
                        list.set(n, ({tostring(v):gsub(activity.getPackageName(), packagename)})[1])
                    elseif user_permission then
                        local p = v:match("%.permission%.([%w_]+)$")
                        -- ProfileInstaller要用 android.permission.DUMP 虽然不知道有什么用
                        if p and (not user_permission[p]) and (not p == "DUMP") then
