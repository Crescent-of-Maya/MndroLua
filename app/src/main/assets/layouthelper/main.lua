...
if luapath:find("%.aly$") then
  ...
  function()
    FastToast.shortSnack(activity,"布局错误或导入包不存在 :(").show()
    --activity.finish()
  end)
  showsave=true
end
...
function getCurr(v)
  ...
  fd_dlg.Title=tostring(v.Class.getName())..(currView.Parent.Tag == null and " (顶部)" or "")
...
function()
  FastToast.shortSnack(activity,"布局出错或导入包不存在 :(").show()
  --activity.finish()
end)
...
--属性列表对话框
fd_dlg=MaterialDialog(activity)
fd_dlg.setPositiveButton("关闭",nil)
fd_dlg.setNegativeButton("编辑属性", {onClick = function()
    --curr
    local e = LuaEditor()
    local 代码配色
    ...
    --配色
    for k,v pairs(代码配色)
      e[k]=Color.parseColor(v)
    end
    local a = ""
    for k, v in pairs(curr)
      if type(k) == "string"
        a = a..k.." = "..
        (type(v) == "string" and
        (v:find("[\"\'\r\n]") and "[===["..v.."]===]" or "'"..v.."'")
    ...
    e.text = (a)
    local d = MaterialDialog(activity)
    .setTitle(currView.class.getName())
    .setView(e)
    .setPositiveButton("保存", {onClick = function()
        local attrs = load("return {" .. e.text.."}")()
        for k, v in pairs(curr)
          if k == 1 continue end
          if attrs[k] == nil
            curr[k] = nil
        ...
        if s then
          ...
          for k,v in pairs(load("return {" .. a.."}")())
            curr[k] = v
          end
          print(l)
        end
    ...
    .setNegativeButton("取消", {onClick=lambda:fd_dlg.show()})
    d.show()
    d.onCancel=lambda:fd_dlg.show()
  end
})
...
--子视图列表对话框
cd_dlg=MaterialDialog(activity)
cd_dlg.setPositiveButton("关闭",{onClick=lambda:fd_dlg.show()})
cd_dlg.setNegativeButton("添加",{onClick=lambda:func["添加"]()})
cd_list=cd_dlg.getListView()
cd_list.onItemClick=function(l,v,p,i)
  if chids[p].Tag == nil then
    FastToast.shortSnack(this, "这是由父布局生成的, 不能编辑哦").show()
    return
  end
  getCurr(chids[p])
  cd_dlg.hide()
end
cd_dlg.onCancel=lambda:fd_dlg.show()

--可选属性对话框
check_dlg=MaterialDialog(activity)
...
check_dlg.onCancel=lambda:fd_dlg.show()
...
func["删除"]=function()
  ...
  if gp==nil then
    FastToast.shortSnack(activity,"哒咩! 不可以删除顶部控件!").show()
    fd_dlg.show()
    return
  end
  MaterialDialog(this)
  ...
  .setNegativeButton("算啦", {onClick=lambda:fd_dlg.show()})
  .show()
end


func["父控件"]=function()
  local p=currView.Parent or {}
  if p.Tag==nil then
    FastToast.shortSnack(activity,"已是顶部控件").show()
    fd_dlg.show()
   else
    getCurr(p)
  end
...
ns={
  "控件","表单控件","适配器控件","高级控件","布局","高级布局",
}

wds={
  {"Button","EditText", "MaterialEditText", "TextView",
    "ImageButton","ImageView"},
  {"CheckBox","RadioButton","ToggleButton","Switch"},
  {"RecyclerView", "ListView","GridView","ViewPager", "PageView","ExpandableListView","Spinner"},
  ...
  {"CardView","RadioGroup","GridLayout",
    "ScrollView","HorizontalScrollView", "SwipeRefreshLayout", "TabLayout", "BottomNavigationView"},
  --{"MaterialButton","FloatingActionButton","BottomNavigationView","Toolbar","DrawerLayout","SwipeRefreshLayout","NavigationView","MaterialCardView","BottomAppBar","AppBarLayout","SearchView",},
}
...
el.onChildClick=function(l,v,g,c)
  ...
  if w[1] == SwipeRefreshLayout then
    -- 防止因为没有内容而空指针错误
    table.insert(w, {LinearLayout})
  end
  table.insert(curr,w)
  local s,l=pcall(loadlayout2,layout.main,{})
  if s then
...
--输入属性对话框
sfd_dlg=MaterialDialog(activity)
fld=EditText(activity)
sfd_dlg.title = "属性"
sfd_dlg.setView(fld)
sfd_dlg.setPositiveButton("确定",{onClick=ok})
sfd_dlg.setNegativeButton("取消",{onClick=lambda:fd_dlg.show()})
sfd_dlg.setNeutralButton("典型值",{onClick=none})
sfd_dlg.onCancel=lambda:fd_dlg.show()
sfd_dlg.create().getButton(DialogInterface.BUTTON_NEUTRAL).onClick=function(v)
...
function onMenuItemSelected(id,item)
  ...
  if t=="复制" then
   ...
   elseif t=="保存" then
    ...
    FastToast.shortSnack(activity,"已保存").show()
...
function onKeyDown(e)
  ...
  if e==4 then
    if now-lastclick>2 then
      FastToast.shortSnack(activity,"再按一次返回").show()
      lastclick=now
      return true
    end
  end
